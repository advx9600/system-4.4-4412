...
public class KeyguardViewMediator {
    ...
    private void doKeyguardLocked(Bundle options) {
        ...
        final boolean requireSim = !SystemProperties.getBoolean("keyguard.no_require_sim",
                false);
        boolean provisioned = mUpdateMonitor.isDeviceProvisioned();
        final IccCardConstants.State state = mUpdateMonitor.getSimState();
         boolean lockedOrMissing = state.isPinLocked()
                || ((state == IccCardConstants.State.ABSENT
                || state == IccCardConstants.State.PERM_DISABLED)
                && requireSim);
         boolean isShowKeyguard = SystemProperties.getBoolean("ro.rpdzkj.isShowKeyguard",false);
        	if(isShowKeyguard){
			           lockedOrMissing = provisioned = true;//add by yangbin
	       	}
